...
from src.model.config import configure_model, configure_training_args#, configure_masked_transformer, configure_classifier, pretrain_training_args, classify_training_args
...
#     if config.data_loc == 'local':
#         train_data, val_data, test_data = fetch_data(config.dataset_code, config)
#         train_data, val_data, test_data = map(lambda data: clean_data(data, config), [train_data, val_data, test_data])
#         _, train_dataset, val_dataset, test_dataset = preprocess.preprocess_finetuning_datasets(train_data, val_data, test_data, config)
    
#     model_config = configure_classifier(config)
#     training_args = classify_training_args(config)
#     early_stopping_callback = setup_early_stopping()

#     if config.finetune and os.path.exists(config.pretrained_model_dir):
#         model = CustomPatchTSMixerForTimeSeriesClassification(model_config).from_pretrained(config.pretrained_model_dir)
#     else:
#         model = CustomPatchTSMixerForTimeSeriesClassification(model_config)

#     train_data, val_data, test_data = fetch_data(config.dataset_code, location='local')
#     train_data = clean_data(train_data, input_columns=config.input_columns, label_column=config.target_columns)
#     val_data = clean_data(val_data, input_columns=config.input_columns, label_column=config.target_columns)
#     test_data = clean_data(test_data, input_columns=config.input_columns, label_column=config.target_columns)
    
#     _, train_dataset, val_dataset, _ = preprocess.preprocess_finetuning_datasets(train_data, val_data, test_data, config.target_columns, config.id_columns)
#     trainer = classifier_trainer(config, model, train_dataset, val_dataset, training_args, early_stopping_callback)
#     evaluate_and_save_model(trainer, test_data, config.save_dir)
